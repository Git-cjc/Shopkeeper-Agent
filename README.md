...
为了把它继续往“更像企业内部问数系统”的方向推进，我已经补上两块更工程化的能力：
...
- 执行超时控制：避免慢 SQL 长时间占用数仓连接
- 节点级错误透传：安全校验失败会明确返回到问数链路和前端

#### 2. 问数评测体系

- 已设计并沉淀 20 条离线评测问题，覆盖聚合、筛选、分组、TopN、品牌、地区、会员等级等典型问数场景
- 支持重复运行的离线评测脚本：`uv run python -m app.scripts.evaluate_query_set`
- 同时统计：
  - SQL 正确率（值正确）
  - 严格结果一致率
  - 列别名一致率
